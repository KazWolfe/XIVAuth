...
require_relative '../../app/lib/patches/device_authorization_grant_patches'
DeviceAuthorizationGrantPatches.apply_patches
...
Doorkeeper::DeviceAuthorizationGrant.configure do
  ...
  verification_uri_complete ->(verification_uri, host_name, device_grant) do
    "#{verification_uri}/#{CGI.escape(device_grant.user_code)}"
  end
end
